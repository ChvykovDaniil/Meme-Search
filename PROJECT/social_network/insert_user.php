...
if(isset($_POST['sign_up'])){
    ...
    if(strlen($pass) < 9 ){
        echo"<script>alert('Пароль должен содержать более 9 символов!')</script>";
        exit();
    }
    ...
    if($check == 1){
        echo "<script>alert('Данный email уже зарегистрирован, пожалуйста попробуйте ввести другой email')</script>";
    ...
    $insert = "insert into users (f_name,l_name,user_name,describe_user,Relationship,user_pass,user_email,user_country,user_gender,user_birthday,user_image,user_cover,user_reg_date,status,posts,recovery_account)
        values('$first_name','$last_name','$username','Стандартный статус','...','$pass','$email','$country','$gender','$birthday','$profile_pic','default_cover.jpg',NOW(),'$status','$posts','Iwanttoputading intheuniverse.')";

    $query = mysqli_query($con, $insert);

    if($query){
        echo "<script>alert('Отлично $first_name, регистрация прошла успешно!.')</script>";
        echo "<script>window.open('signin.php', '_self')</script>";
    }
    else{
        echo "<script>alert('Регистрация провалилась, попробуй еще раз!')</script>";
        echo "<script>window.open('signup.php', '_self')</script>";
    }
}
?>
