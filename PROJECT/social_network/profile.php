...
echo"
			<div>
				...
				<ul class='nav pull-left' style='position:absolute;top:10px;left:40px;'>
					<li class='dropdown'>
						<button class='dropdown-toggle btn btn-default' data-toggle='dropdown'>Поменять фон</button>
						<div class='dropdown-menu'>
							<center>
							<p>Нажмите <strong>Выбрать фон</strong> и затем нажмите <br> <strong>Обновить фон</strong></p>
							<label class='btn btn-info'> Выбрать фон
							<input type='file' name='u_cover' size='60' />
							</label><br><br>
							<button name='submit' class='btn btn-info'>Изменить фон</button>
							</center>
						</div>
					</li>
				</ul>
			...
			<div id='profile-img'>
				...
				<label id='update_profile'> Выбрать аватар
				<input type='file' name='u_image' size='60' />
				</label><br><br>
				<button id='button_profile' name='update' class='btn btn-info'>Изменить аватар</button>
				</form>
			</div><br>
			";
?>
...
if(isset($_POST['submit'])){
    ...
    if($u_cover==''){
        echo "<script>alert('Пожалуйста, выберите фоновое изображение')</script>";
        ...
        if($run){
            echo "<script>alert('Ваш фон обновлен')</script>";
            echo "<script>window.open('profile.php?u_id=$user_id' , '_self')</script>";
        }
    }
...
?>
    ...
    if(isset($_POST['update'])){
        ...
        if($u_image==''){
            echo "<script>alert('Пожалуйста, выберите изображение аватара при нажатии на изображение вашего профиля')</script>";
            echo "<script>window.open('profile.php?u_id=$user_id' , '_self')</script>";
            exit();
        }else{
            ...
            if($run){
                echo "<script>alert('Ваш аватар обновлен')</script>";
                echo "<script>window.open('profile.php?u_id=$user_id' , '_self')</script>";
            }
        }
...
<div class="row">
    ...
    <div class="col-sm-2" style="background-color: #e6e6e6;text-align: center;left: 0.8%;border-radius: 5px;">
        <?php
        echo"
			<center><h2><strong>Информация</strong></h2></center>
			<center><h4><strong>$first_name $last_name</strong></h4></center>
			<p><strong><i style='color:grey;'>$describe_user</i></strong></p><br>
			<p><strong>Семейное положение: </strong> $Relationship_status</p><br>
			<p><strong>Место проживания: </strong> $user_country</p><br>
			<p><strong>Зарегистрирован: </strong> $register_date</p><br>
			<p><strong>Пол: </strong> $user_gender</p><br>
			<p><strong>День рождения: </strong> $user_birthday</p><br>
		";
        ?>
    </div>
    <div class="col-sm-6">
        ...
        while($row_posts = mysqli_fetch_array($run_posts)){
            ...
            if($content == "No" && strlen($upload_image) >= 1){
                ...
                if($u_email != $user_email){
                    ...
                    <a href='edit_post.php?post_id=$post_id' style='float:right;'><button class='btn btn-info'>Изменить</button></a>
