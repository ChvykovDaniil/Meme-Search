...
function insertPost(){
    if(isset($_POST['sub'])){
        ...
        if(strlen($content) > 250){
            echo "<script>alert('Не используйте более 250 символов!')</script>";
            ...
            if(strlen($upload_image) >= 1 && strlen($content) >= 1){
                ...
                if($run){
                    echo "<script>alert('Ваша публикация была добавлена!')</script>";
                    echo "<script>window.open('home.php', '_self')</script>";
                ...
                if($upload_image=='' && $content == ''){
                    echo "<script>alert('Произошла ошибка при загрузке!')</script>";
                    echo "<script>window.open('home.php', '_self')</script>";
                }else{
                    if($content==''){
                        move_uploaded_file($image_tmp, "imagepost/$upload_image.$random_number");
				...
				<div id='posts' class='col-sm-6'>
					<div class='row'>
						...
						<div class='col-sm-6'>
							...
							<h4><small style='color:black;'>Пост обновлен <strong>$post_date</strong></small></h4>
						</div>
						<div class='col-sm-4'>
						</div>
					</div>
    ...
    if(isset($_GET['post_id'])){
        ...
        if($p_id != $post_id){
                ...
                <div class='col-md-6 col-md-offset-3'>
                    <div class='panel panel-info'>
                        <div class='panel-body'>
                            ...
                            <textarea placeholder='Напиши свой комментарий тут' class='pb-cmnt-textarea' name='comment'></textarea>  
                            <button class='btn btn-info pull-right' name='reply'>Прокомментировать</button>                            
                            </form>
                        </div>               
                    </div>                
                </div>
            ...
            if(isset($_POST['reply'])){
                ...
                if($comment == ""){
                    echo "<script>alert('Введите свой комментарий!')</script>";
                    ...
                    echo "<script>alert('Ваш комментарий добавлен!')</script>";
                    echo "<script>window.open('single.php?post_id=$post_id', '_self')</script>";
                }
            }
        }
            ...
            <div class='col-sm-6'>
                <div class='row' id='find_people'>
                    <div class='col-sm-4'>
                        ...
                        <img src='users/$user_image' width='200px' height='170px' title='$username' style='float: left; ,margin: 1px;'/>
                        </a>
                    </div><br><br>
                    <div class='col-sm--6'>
